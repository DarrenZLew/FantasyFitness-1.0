import data from "../fakeData/scoreSheet-fakedata.json";
...
const indexActivity = data.users[0].activities.findIndex(
  activity => activity.name === data.currentActivity.name
);
...
if (data.view === "score") {
  usersData.map(user => {
    if (user.hasOwnProperty("activityAmountMin")) {
      user.activityAmount =
        user.activityAmountMin / 60 * data.currentActivity.points;
...
const initialState = {
  users: usersData.sort((a, b) => {
    if (
      a.hasOwnProperty("activityAmountMin") &&
      a.activityAmountMin !== undefined
    ) {
  ...
  switch (action.type) {
    case ScoreSheetActions.Types.UpdateActivity:
      if (action.payload.success) {
        ...
        const indexActivity = data.users[0].activities.findIndex(
          activity => activity.name === action.payload.activity
        );
        ...
        const indexUser = newState.users.findIndex(
          user => user.username === newState.currentUser.name
        );
        ...
        if (data.view === "score") {
          newState.users.map(user => {
            if (user.hasOwnProperty("activityAmountMin")) {
              user.activityAmount =
                user.activityAmountMin / 60 * newState.currentActivity.points;
              user.activityAmountMin = undefined;
            } else {
              user.activityAmount =
                user.activityAmount * newState.currentActivity.points;
            }
          });
        }
    ...
    case ScoreSheetActions.Types.UpdateScoreView:
      if (action.payload.success) {
        ...
        if (action.payload.view === "score") {
          ...
          newState.users.map(user => {
            if (
              user.hasOwnProperty("activityAmountMin") &&
              user.activityAmountMin === undefined
            ) {
              user.activityAmountMin =
                user.activityAmount * 60 / newState.currentActivity.points;
